...
void pipeline(tokenlist *tokens, int pipe_count, bool background,
     job_list_t *jobs, const char *original_cmd);
...
void execute_command(char *cmd_path, tokenlist *tokens, bool background, 
    job_list_t *jobs,char *in_file, char *out_file, const char *original_cmd) {
    pid_t pid = fork();
...
bool handle_builtin(tokenlist *tokens, job_list_t *jobs,
     command_history_t *history, bool *should_exit) {
    if (tokens->size == 0) {
        return false;
    }
...
void pipeline(tokenlist *tokens, int pipe_count,bool background, 
 ...
 if (!background) {
        ...
        add_job(jobs, pids[cmd_count - 1], original_cmd);
        printf("[%d] %d\n",
...
int main(void) {
	...
	if(pipe_count > 0){
	  ...
	  if(pipe_count > 2){
       ...
       pipeline(tokens,pipe_count,is_background,&jobs, original_input);
        ...
        if (tokens->size > 0) {
            ...
            if (!handle_builtin(tokens, &jobs, &history, &should_exit)) {
                ...
                if (cmd_path != NULL) {
                    ...
                    add_to_history(&history, original_input);
                    execute_command(cmd_path, tokens, is_background, &jobs,in_file,out_file, original_input);
                    free(cmd_path);
                } else {
